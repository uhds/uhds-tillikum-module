...
class Holds extends ReportForm implements EntityManagerAwareInterface
{
    protected $em;
    ...
    public function init()
    {
        ...
        $date = new \Tillikum_Form_Element_Date(
            'date',
            array(
                'label' => 'For which date would you like to see holds?',
                'required' => true,
            )
        );
        ...
        $facilityGroups = new \Zend_Form_Element_Multiselect(
            'facility_groups',
            array(
                'label' => 'Which buildings would you like to see holds for?',
                'multiOptions' => array(),
                'required' => true,
            )
        );
